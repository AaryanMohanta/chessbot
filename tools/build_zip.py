...
def check_imports(shipped_files: list[str], errors: list[str], source_overrides: dict[str, str] | None = None) -> None:
    """``source_overrides``: {filename: source_text} for files whose
    content isn't read from disk -- currently just cb_build_tag.py, which
    build() generates in-memory rather than writing to the repo root (see
    generate_build_tag_source's docstring for why)."""
    ...
    for filename in shipped_files:
        if filename in source_overrides:
            source = source_overrides[filename]
        else:
...
def generate_build_tag_source(tag: str, flags: dict[str, str]) -> str:
    """Returns the source text for cb_build_tag.py -- see that file's own
    generated docstring for why it exists: identifying which build/config
    played a given ladder game (see agent.py's startup log line and
    ...
    Pure (no I/O) on purpose -- build() writes this straight into the
    output zip via zipfile.writestr, NEVER to the live repo root. An
    earlier version wrote it to REPO_ROOT/cb_build_tag.py first and zipped
    that path like every other shipped file; since agent.py (run locally
    by ratings/run_variant_round_robin.py, ratings/sprt.py, and any other
    local self-play testing) also imports cb_build_tag from that same
    repo-root location, running a build while local testing was in
    progress would silently overwrite the flags that testing depended on
    mid-run. Hit twice in one session (2026-09) -- the second time
    injecting CB_NB_ENABLE_CORRHIST=1 into a live SPRT run's "baseline"
    side partway through and visibly skewing its LLR (a clean upward trend
    reversed right at the moment the contaminating build ran). Generating
    the content in-memory and writing it only into the zip's own bytes
    makes that entire bug class structurally impossible: building a
    submission can no longer touch anything a concurrently-running local
    process might read. See generate_build_tag_file below for writing
    this to an arbitrary path (tests use an isolated tmp path; nothing
    in build() itself should ever pass REPO_ROOT/cb_build_tag.py)."""
    lines = [
...
def generate_build_tag_file(tag: str, flags: dict[str, str], path: Path = BUILD_TAG_PATH) -> None:
    """Writes generate_build_tag_source's output to an explicit path --
    used by tests (against an isolated tmp path) and by anyone who wants a
    real cb_build_tag.py on disk for local interactive use (e.g. manually
    testing one flag combo by hand). build() itself does NOT call this --
    see generate_build_tag_source's docstring for why."""
    path.write_text(generate_build_tag_source(tag, flags), encoding="utf-8")
...
def build(
    ...
    resolved_tag = tag if tag is not None else default_build_tag()
    build_tag_source = generate_build_tag_source(resolved_tag, flags or {})

    check_imports(SHIPPED_PY_FILES, errors, source_overrides={"cb_build_tag.py": build_tag_source})
    ...
    # cb_build_tag.py is handled separately below (generated in-memory,
    # never read from or written to the repo root) -- see
    # generate_build_tag_source's docstring for why.
    disk_py_files = [REPO_ROOT / f for f in SHIPPED_PY_FILES if f != "cb_build_tag.py"]
    shipped_paths = disk_py_files + data_files + tablebase_files + weight_files
    existing_paths = [p for p in shipped_paths if p.exists()]

    check_forbidden_files(existing_paths, errors)

    build_tag_bytes = len(build_tag_source.encode("utf-8"))
    total_size = sum(p.stat().st_size for p in existing_paths) + build_tag_bytes
    ...
    with zipfile.ZipFile(output_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr("cb_build_tag.py", build_tag_source)

    print(f"OK: wrote {output_path} ({total_size} bytes unzipped, {len(existing_paths) + 1} files, "
          f"build_tag={resolved_tag!r})")
    print("Files:")
    for path in existing_paths:
        print(f"  {path.name}  ({path.stat().st_size} bytes)")
    print(f"  cb_build_tag.py  ({build_tag_bytes} bytes) [generated in-memory, not written to repo root]")
